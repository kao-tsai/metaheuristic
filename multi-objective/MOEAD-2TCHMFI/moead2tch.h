#ifndef __MOEAD2TCH_H_INCLUDED__
#define __MOEAD2TCH_H_INCLUDED__
#include <iostream>
#include <string>
#include <stdio.h>
#include <stdlib.h>
...
class moead2tch{
	public:
		...
		moead2tch(int,int,int,int,double,double,string);
		void run();
	private:
		void init(int);
        int C(int,int);
...
moead2tch::moead2tch(int xNumRuns,
int xNumIter,
int xH,
int xT_num,
...
// int moead2tch::C(int up,int down){
//     if(down==0)
//         return 1;
//     else if(down==1)
//         return up;
...
void moead2tch::init(int cur_run){
    current_sol.clear();
...
void moead2tch::generate_weight_vector(){
...
void moead2tch::update_z(int cur_i,solution& new_sol_fit){
    for(int i=0;i<obj_num;i++){
        nadir_z[i]=-inf;
...
void moead2tch::quick_sort(solution& value,vector<int>& this_rank,int left,int right){
    int index;
    int int_tmp;
    double double_tmp;
    int i, j;
...
double moead2tch::cal_euDis(solution& t1,solution& t2){
    double sum=0;
    double len=t1.size();
    for(int i=0;i<len;i++)
...
moead2tch::solution moead2tch::crossover(solution& x1,solution& x2){
...
void moead2tch::mutation(solution& x1){
    int j;
...
moead2tch::solution moead2tch::transition(int cur_i){
    int index1,index2;
    index1=rand()%T_num;
    do{
        index2=rand()%T_num;
...
void moead2tch::update_neighbor_sol(int cur_i,solution& new_sol,solution& new_sol_fit){
...
double moead2tch::TCH(solution& sol_fit,solution& cur_wei){
    //Without Normalized
...
double moead2tch::PBI(solution& sol_fit,solution& cur_wei){
    double dis1,dis2;
...
void moead2tch::run(){
    ...
    for(int i=0;i<numRuns;i++){
        ...
        // //輸出該RUN的所有解
        ofstream output_obj;
        output_obj.open("pareto/"+func_name+"/moead2tch/"+func_name+"_moead2tch_"+to_string(i)+".txt");
          for(int k=0;k<population_num;k++){
...
moead2tch::population moead2tch::fitness(population& all_sol) {
    if(func_name=="SCH")
